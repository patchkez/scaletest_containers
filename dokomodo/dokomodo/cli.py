...
@click.command('generate_json_docker_compose',
    short_help='PROD - Generates docker-compose file with all assetchains from json')
# @click.option('-b', '--branch', required=True, type=click.Choice(['development', 'production',
#    'test']), prompt=True)
@click.option('-a', '--asset', required=False, help='name of assetchain in capital \
    letters e.g. SUPERNET')
...
@pass_config
def generate_json_docker_compose(ctx, asset, image, notarize):
    """ TODO """
    filename = 'docker-compose-assets-scale8000' + 'newscale' + '.yml'
    dirname = "./"
    click.echo('Writing new docker compose file into: {}'.format(filename))
    ...
    template = env.get_template('docker-compose-scale-template.conf.j2')
    templatized_config = template.render(items=data, seed_ip=ctx.seed_ip2, mined=ctx.mined_coins,
            btcpubkey=ctx.btcpubkey, image_name=image, notarize=notarize)
    ctx.write_config(dirname, filename=filename, templatized_config=templatized_config)


@click.command('generate_docker_compose_for_mylo',
    short_help='PROD - Generates docker-compose file with all assetchains from json')
# @click.option('-b', '--branch', required=True, type=click.Choice(['development', 'production',
#    'test']), prompt=True)
@click.option('-a', '--asset', required=False, help='name of assetchain in capital \
...
@click.option('-n', '--notarize', required=True, is_flag=True, help='notarize chain, \
    set to True or False')
@pass_config
def generate_docker_compose_for_mylo(ctx, asset, image, notarize):
    """ TODO """
    filename = 'docker-compose-dynamodb-test' + '.yml'
    dirname = "./"
    click.echo('Writing new docker compose file into: {}'.format(filename))
    ...
    template = env.get_template('docker-compose-for-mylo-template.conf.j2')
    templatized_config = template.render(items=data, seed_ip=ctx.seed_ip2, mined=ctx.mined_coins,
            btcpubkey=ctx.btcpubkey, image_name=image, notarize=notarize)
    ctx.write_config(dirname, filename=filename, templatized_config=templatized_config)


@click.command('generate_k8n_template',
    short_help='PROD - Generates AWS K8N definition file with all assetchains from json')
@click.option('-a', '--asset', required=False, help='name of assetchain in capital \
    letters e.g. SUPERNET')
@click.option('-i', '--image', required=True, help='name of image used for assetchains, \
    it must match image name you use for komodod e.g. kmdplatform_komodod_dev or \
...
def generate_k8n_template(ctx, asset, image, notarize):
    ...
    template = env.get_template('aws-k8n-2pods-template.conf.j2')
    dirname = "./"
...
cli.add_command(generate_json_docker_compose)
cli.add_command(generate_docker_compose_for_mylo)
...
cli.add_command(generate_k8n_template)
